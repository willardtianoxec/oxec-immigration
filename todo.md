...
- [ ] 创建单元测试 - 验证所有CLB转换和评分场景
- [ ] 测试所有场景 - 验证两个方案的计算准确性
- [ ] 创建检查点 - 完成CRS Calculator重构


## Calculator 视觉回归与逻辑重构 (Current Sprint - Phase 20)
- [ ] 分析当前设计并规划视觉回归方案 - 对齐/bccalculator的深蓝色渐变设计
- [ ] 重构表单结构为三个分区 - 人力资源、可转移技能、附加分项
- [ ] 实现人力资源因素分区 - 年龄、学历、语言、工作经验、配偶模块
- [ ] 添加配偶定义注解 - "配偶指已婚配偶、同居一年以上伴侣或事实配偶"
- [ ] 实现可转移技能分区 - 学历与语言组合、学历与经验组合等
- [ ] 实现附加分项分区 - 兄弟姐妹、省提名、法语加分、教育加分
- [ ] 优化视觉样式 - 标题36磅、背景渐变、卡片式容器、垂直排列
- [ ] 测试所有交互和计算功能 - 验证三个分区的得分小计和总分
- [ ] 保存检查点 - 完成/calculator视觉回归和逻辑重构
